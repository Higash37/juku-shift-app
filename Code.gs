...
function getMembers() {
  ...
  const sheet =
    spreadsheet.getSheetByName("members");
...
function saveShifts(request) {
  ...
  if (!shiftsSheet) {
    throw new Error(
      "shifts_DBシートが見つかりません。"
    );
  }

  if (!lessonsSheet) {
    throw new Error(
      "lessons_DBシートが見つかりません。"
    );
  }
